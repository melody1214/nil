...
func (h *Handler) needRebalance() bool {
	q := fmt.Sprintf(
		`
		SELECT
			chain,
...
func (h *Handler) rebalance() error {
...
func (h *Handler) rebalanceSpeedGroup(speed string) error {
	q := fmt.Sprintf(
		`
		SELECT
			volume_id,
...
func (h *Handler) doRebalance(target *Volume, group []*Volume) error {
...
func (h *Handler) newLocalChain(primary *Volume, vols []*Volume) error {
	const localChainNum = 4
